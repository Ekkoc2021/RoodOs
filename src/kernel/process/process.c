...
uint16_t createProcess2(uint16_t weight, uint16_t argsLength, char *name, ...)
{
    ...
    */
    ...
    // 分配所需内存
    uint32_t me = mallocMultpage_u(&market, 10); // 没有释放这里的内存
    StackInfo *s = (StackInfo *)(pcb->esp0 - sizeof(StackInfo));
    // 读取程序进入内存
    ...
    // 初始化0特权级栈
    initStack(s, 0, (me) + 4096 * (10) - 32 - argsLength * sizeof(char) - 4);
...
char file_buff[512]; // 用于createProcess3的缓存file_path的数据
uint16_t createProcess3(uint16_t weight, char *exec_file_path, uint16_t argsLength, ...)
{

    // path中提取名称
    char name[20];
    strcpy_(file_buff, exec_file_path);
    exec_file_path = file_buff;
